...
interface CartItemsListProps {
  cartItems: CartItem[];
  appliedDiscounts: AppliedDiscount[]; // Changed to array
  onRemoveItem: (productId: string) => void;
  onUpdateQuantity: (productId: string, quantity: number) => void;
}
...
export const CartItemsList: React.FC<CartItemsListProps> = ({
  ...
  const getDiscountedPrice = (productId: string) => {
    const discountItem = appliedDiscounts
      .flatMap((d) => d.discountedItems)
      .find((di) => di.productId === productId);
    return discountItem ? discountItem.discountedPrice : null;
  };
  ...
  return (
    <div className="flex flex-col items-center w-1/2">
      {cartItems.map((item) => {
        ...
        const appliedDiscount = appliedDiscounts.find((d) =>
          d.discountedItems.some((di) => di.productId === item.productId)
        );
            ...
            <div className="flex flex-col justify-center items-center">
              ...
              {discountedPrice !== null ? (
                <div className="flex items-center space-x-2">
                  ...
                  {appliedDiscount && (
                    <p className="text-xs text-gray-500">({appliedDiscount.code})</p>
                  )}
                </div>
              ) : (
                <p className="text-gray-700">₹{item.price.toLocaleString()}</p>
