...
export const ProductTable: React.FC<ProductTableProps> = ({
  ...
  <div className="bg-white p-4 rounded-lg shadow-md">
          ...
          <th className="p-2">
            <input
              ...
              onChange={(e) =>
                handleSelectProduct(
                  e.target.checked
                    ? productsData?.data?.products.map((p) => p._id!) || []
                    : []
                )
              }
              checked={
    ...
    <div className="flex justify-between items-center mt-4">
      ...
      <span
        aria-label={`Products page ${productsData?.data?.currentPage} of ${productsData?.data?.totalPages}`}
      >
        Page {productsData?.data?.currentPage} of{" "}
        {productsData?.data?.totalPages} (Total: {productsData?.data?.total})
      </span>
      <motion.button
        onClick={() =>
          setPage(Math.min(page + 1, productsData?.data?.totalPages || 1))
        }
